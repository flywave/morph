...
func generateNewDriver(driverName string) error {
	...
	f.ImportNames(map[string]string{
		"github.com/flywave/morph/drivers": "drivers",
		"github.com/flywave/morph/models":  "models",
	})
	...
	f.Func().Id("init").Params().Block(
		Qual("github.com/flywave/morph/drivers", "Register").Call(Lit(driverName), Id("&"+driverName).Values()),
	)
	...
	f.Line()
	f.Func().Params(Id("driver").Id("*" + driverName)).Id("Apply").Params(Id("migration").Op("*").Qual("github.com/flywave/morph/models", "Migration")).Error().BlockFunc(func(g *Group) {
		g.Comment("Implement the functionality for apply the migration onto the storage.")
		g.Panic(Lit("implement me"))
	})

	f.Line()
	f.Func().Params(Id("driver").Id("*"+driverName)).Id("AppliedMigrations").Params().Params(Op("[]*").Qual("github.com/flywave/morph/models", "Migration"), Error()).BlockFunc(func(g *Group) {
